...
metBx = ROOT.TH1F("metBx", "", 4000, 0.0, 4000.0)
muonBx = ROOT.TH1F("muonBx", "", 4000, 0.0, 4000.0)
# make output dir
...
parser.add_option(
    ...
    type="str",
    default="nojson",
    help="text FILE with the LS range in json format",
    metavar="FILE",
)
parser.add_option(
    "-s",
    ...
    type="str",
    default="nostr",
    help="STRING used to name the output",
    metavar="STRING",
)
parser.add_option(
    "-f",
    ...
    type="str",
    default="custom",
    help="ARG='custom' (default option), 'RAW' or 'L1Accept', use 'custom' if you're running on STEAM-made files, 'RAW' if you're running on raw data, 'L1Accept' if you're running on L1Accept data",
    metavar="ARG",
)
parser.add_option(
    "-m",
    ...
    type="str",
    default="nomaps",
    help="ARG='nomaps' (default option, don't use maps to get dataste/groups/etc. rates), 'somemaps' (get dataste/groups/etc. rates but with no study of dataset merging), 'allmaps' (get dataste/groups/etc. rates and also study dataset merging)",
    metavar="ARG",
)
parser.add_option(
    "-M",
    ...
    type="int",
    default=-1,
    help="maximum number of events to be processed (default -1 to process all events)",
    metavar="INT",
)
parser.add_option(
    "-x",
    ...
    type="str",
    default="",
    help="list of triggers to exclude from pure rate calculation",
    metavar="FILE",
)
...
if (
    opts.inputFile == "noroot"
    or opts.finalString == "nostr"
    or opts.jsonFile == "nojson"
):
    print(error_text)
    print(help_text)
    sys.exit(2)
...
if opts.exclude != "":
    with open(opts.exclude) as f:
        triggersToExcludeFromPureRate = f.read().splitlines()
        print("\nExcluding the following triggers from pure rate calculation:")
        print(", ".join(triggersToExcludeFromPureRate))
...
for event in events:
    ...
    if nLoop < 1:
        for name in names.triggerNames():
            ...
            if ("HLTriggerFirstPath" in name) or ("HLTriggerFinalPath" in name):
                continue
            myPaths.append(name)
            if bUseMaps:
                ...
                if actualKey in triggersDatasetMap:
                    datasets.update(
                        {str(strippedTrigger): triggersDatasetMap[actualKey]}
                    )
                    datasetKnown = True
                else:
    ...
    if isL1Accept:
        ...
        for triggerName in myPaths:
            if not "DST_Physics_v" in triggerName:
                continue
    ...
    triggerCounts = 0
    firedTriggers = []
    firedTriggersNotExcluded = []
    for i in range(0, len(myPaths)):
        triggerCountsBool[myPaths[i]] = False
    if bUseMaps:
        datasetsLatestCounts = primaryDatasetCounts.fromkeys(
            list(primaryDatasetCounts.keys()), 0
        )
        if opts.maps == "allmaps":
            newDatasetsLatestCounts = newDatasetCounts.fromkeys(
                list(newDatasetCounts.keys()), 0
            )
    ...
    for triggerName in myPaths:
        ...
        if checkTriggerIndex(triggerName, index, names.triggerNames()):
            ...
            if triggerBits.product().accept(index):
                if "HLT_PFMET" in str(triggerName):
                    metBx.Fill(event.object().bunchCrossing() * 1.0)

                if "HLT_IsoMu24" in str(triggerName):
                    muonBx.Fill(event.object().bunchCrossing() * 1.0)
                ...
                if not bUseMaps:
                    if triggerName.startswith("HLT_"):
                        triggerCounts += 1
                        firedTriggers.append(triggerName)
                        if triggerName not in triggersToExcludeFromPureRate:
                            firedTriggersNotExcluded.append(triggerName)
                    ...
                    if strippedTrigger in list(datasets.keys()):
                        for dataset in datasets[strippedTrigger]:
                            if datasetsLatestCounts[dataset] == 0:
                                primaryDatasetCounts[dataset] = (
                                    primaryDatasetCounts[dataset] + 1
                                )
                    ...
                    if strippedTrigger in list(groups.keys()):
                        for group in groups[strippedTrigger]:
                            if not physicsStreamOK(strippedTrigger):
                                continue
                    ...
                    if kPassedEventAnalysis == False:
                        if belongsToPAG(strippedTrigger) and physicsStreamOK(
                            strippedTrigger
                        ):
                            if ("backup" in types[strippedTrigger]) or (
                                "signal" in types[strippedTrigger]
                            ):
                                nPAGAnalysisPath += 1
                    ...
                    if kPassedEventMisc == False:
                        if not (
                            parkingStreamOK(strippedTrigger)
                            or scoutingStreamOK(strippedTrigger)
                            or physicsStreamOK(strippedTrigger)
                        ):
                            nPassed_Misc += 1
                            kPassedEventMisc = True
        ...
        iPath = iPath + 1
    for trigger in myPaths:
        if not triggerCountsBool[trigger]:
            continue
        myPassedEvents[trigger][0] += 1
        if len(firedTriggersNotExcluded) <= 1 and trigger.startswith("HLT_"):
            # increment pure rate count (if other triggers are not excluded)
    ...
    if bUseMaps:
        ...
        for dataset1 in primaryDatasetList:
            if datasetsLatestCounts[dataset1] == 0:
                continue
            for dataset2 in primaryDatasetList:
                if datasetsLatestCounts[dataset2] == 0:
                    continue
            ...
            for trigger in myPaths:
                if not triggerCountsBool[trigger]:
                    continue
                strippedTrigger = trigger.rstrip("0123456789")
                triggerDatasetCorrMatrix[dataset1][
                    strippedTrigger
                ] += 1  # somehow this is increasing the non-pure rate when dataset1 = unassigned
                if strippedTrigger in list(datasets.keys()):
                    if (dataset1 in datasets[strippedTrigger]) and datasetsLatestCounts[
                        dataset1
                    ] > 1:
                        triggerDatasetCorrMatrix[dummy_nonpure][strippedTrigger] += 1
        if opts.maps == "allmaps":
            for dataset1 in newDatasetList:
                if newDatasetsLatestCounts[dataset1] == 0:
                    continue
                for dataset2 in newDatasetList:
                    if newDatasetsLatestCounts[dataset2] == 0:
                        continue
                        ...
                        if newDatasetsLatestCounts[dataset1] > 1:
                            if dataset1 in datasets[strippedTrigger]:
                                ...
                                for old_dataset in list(newDatasetMap.keys()):
                                    if not (dataset1 in newDatasetMap[old_dataset]):
                                        continue
                                    if old_dataset in datasets[strippedTrigger]:
                                        bUseDummy = True
                                        break
                    if bUseDummy:
                        triggerNewDatasetCorrMatrix[dummy_nonpure][strippedTrigger] += 1

        if len(myGroupFired) == 1:
            groupCountsPure[myGroupFired[0]] = groupCountsPure[myGroupFired[0]] + 1

        for group in myGroupFired:
            groupCountsShared[group] = groupCountsShared[group] + 1.0 / len(
                myGroupFired
            )
...
if atLeastOneEvent:
    ...
    misc_path_file.write(
        "Path, Groups, Type, Total Count, Total Rate (Hz), Pure Count, Pure Rate (Hz)\n"
    )
    misc_path_file.write(
        "Total Misc, , , " + str(nPassed_Misc) + ", " + str(nPassed_Misc) + "\n"
    )

    root_file = ROOT.TFile("%s/histos.%s.root" % (outputDir, final_string), "RECREATE")
    if bUseMaps:
        physics_path_file = open(
            "%s/output.path.physics.%s.csv" % (outputDir, final_string), "w"
        )
        ...
        physics_path_file.write(
            "Total Analysis Physics, , , "
            + str(nPAGAnalysisPath)
            + ", "
            + str(nPAGAnalysisPath)
            + "\n"
        )

        scouting_path_file = open(
            "%s/output.path.scouting.%s.csv" % (outputDir, final_string), "w"
        )
        ...
        scouting_path_file.write(
            "Total Scouting, , , "
            + str(nPassed_Scouting)
            + ", "
            + str(nPassed_Scouting)
            + "\n"
        )

        parking_path_file = open(
            "%s/output.path.parking.%s.csv" % (outputDir, final_string), "w"
        )
        ...
        triggerDataset_histo = ROOT.TH2F(
            "trigger_dataset_corr",
            "Trigger-Dataset Correlations",
            len(primaryDatasetList) + 1,
            0,
            len(primaryDatasetList) + 1,
            len(myPaths),
            0,
        ...
        datasetDataset_histo = ROOT.TH2F(
            "dataset_dataset_corr",
            "Dataset-Dataset Correlations",
            len(primaryDatasetList),
            0,
            len(primaryDatasetList),
            len(primaryDatasetList),
            0,
        ...
        if opts.maps == "allmaps":
            triggerNewDataset_histo = ROOT.TH2F(
                "trigger_newDataset_corr",
                "New Trigger-Dataset Correlations",
                len(newDatasetList) + 1,
                0,
                len(newDatasetList) + 1,
                len(myPaths),
                0,
            ...
            newDatasetNewDataset_histo = ROOT.TH2F(
                "newDataset_newDataset_corr",
                "New Dataset-Dataset Correlations",
                len(newDatasetList),
                0,
                len(newDatasetList),
                len(newDatasetList),
            ...
            triggerNewDataset_file = open(
                "%s/output.trigger_newDataset_corr.%s.csv" % (outputDir, final_string),
                "w",
            )
            newDatasetNewDataset_file = open(
                "%s/output.newDataset_newDataset_corr.%s.csv"
                % (outputDir, final_string),
                "w",
            )

        triggerDataset_file = open(
            "%s/output.trigger_dataset_corr.%s.csv" % (outputDir, final_string), "w"
        )
        datasetDataset_file = open(
            "%s/output.dataset_dataset_corr.%s.csv" % (outputDir, final_string), "w"
        )
        ...
        if not bUseMaps:
            misc_path_file.write(
                "{}, {}, {}, {}, {}, {}, {}".format(
                    ...
                    group_string,
                    type_string,
                    myPassedEvents[trigger][0],
                    myPassedEvents[trigger][0],
                    myPassedEvents[trigger][1],
                    myPassedEvents[trigger][1],
                )
            )
            misc_path_file.write("\n")
            ...
            for dataset in primaryDatasetList:
                triggerDataset_file.write(
                    ", " + str(triggerDatasetCorrMatrix[dataset][strippedTrigger])
                )
                triggerDataset_histo.GetXaxis().SetBinLabel(j + 1, dataset)
                triggerDataset_histo.SetBinContent(
                    j + 1, i + 1, triggerDatasetCorrMatrix[dataset][strippedTrigger]
                )
                j += 1
            triggerDataset_histo.GetXaxis().SetBinLabel(j + 1, dummy_nonpure)
            triggerDataset_histo.SetBinContent(
                j + 1, i + 1, triggerDatasetCorrMatrix[dummy_nonpure][strippedTrigger]
            )
            triggerDataset_file.write(
                ", " + str(triggerDatasetCorrMatrix[dummy_nonpure][strippedTrigger])
            )
            triggerDataset_file.write("\n")
            ...
            if opts.maps == "allmaps":
                ...
                for dataset in newDatasetList:
                    triggerNewDataset_file.write(
                        ", "
                        + str(triggerNewDatasetCorrMatrix[dataset][strippedTrigger])
                    )
                    triggerNewDataset_histo.GetXaxis().SetBinLabel(j + 1, dataset)
                    triggerNewDataset_histo.SetBinContent(
                        j + 1,
                        i + 1,
                        triggerNewDatasetCorrMatrix[dataset][strippedTrigger],
                    )
                    j += 1
                ...
                triggerNewDataset_histo.SetBinContent(
                    j + 1,
                    i + 1,
                    triggerNewDatasetCorrMatrix[dummy_nonpure][strippedTrigger],
                )
                triggerNewDataset_file.write(
                    ", "
                    + str(triggerNewDatasetCorrMatrix[dummy_nonpure][strippedTrigger])
                )
                triggerNewDataset_file.write("\n")
    ...
    if bUseMaps:
        ...
        physics_dataset_file = open(
            "%s/output.dataset.physics.%s.csv" % (outputDir, final_string), "w"
        )
        misc_dataset_file = open(
            "%s/output.dataset.misc.%s.csv" % (outputDir, final_string), "w"
        )
        ...
        for key in primaryDatasetList:
            ...
            for trigger in myPaths:
                strippedTrigger = trigger.rstrip("0123456789")
                if not strippedTrigger in list(datasets.keys()):
                    continue
                if physicsStreamOK(strippedTrigger) and (
                    key in datasets[strippedTrigger]
                ):
            ...
            for key2 in primaryDatasetList:
                j += 1
                if primaryDatasetCounts[key] > 0:
                    datasetDatasetCorrMatrix[key2][key] = datasetDatasetCorrMatrix[
                        key2
                    ][
                        key
                    ]  # /primaryDatasetCounts[key]
                datasetDataset_file.write(
                    ", " + str(datasetDatasetCorrMatrix[key2][key])
                )
                datasetDataset_histo.GetXaxis().SetBinLabel(j, key2)
                datasetDataset_histo.SetBinContent(
                    j, i, datasetDatasetCorrMatrix[key2][key]
                )
            datasetDataset_file.write("\n")

        if opts.maps == "allmaps":
            newDataset_file = open(
                "%s/output.newDataset.physics.%s.csv" % (outputDir, final_string), "w"
            )
            ...
            for key in newDatasetList:
                ...
                for trigger in myPaths:
                    ...
                    if physicsStreamOK(strippedTrigger):
                        if key in datasets[strippedTrigger]:
                            ...
                            for old_dataset in list(newDatasetMap.keys()):
                                if not (key in newDatasetMap[old_dataset]):
                                    continue
                                if old_dataset in datasets[strippedTrigger]:
                                    isPhysicsDataset = True
                                    break
                ...
                for key2 in newDatasetList:
                    j += 1
                    newDatasetNewDataset_file.write(
                        ", " + str(newDatasetNewDatasetCorrMatrix[key2][key])
                    )
                    newDatasetNewDataset_histo.GetXaxis().SetBinLabel(j, key2)
                    newDatasetNewDataset_histo.SetBinContent(
                        j, i, newDatasetNewDatasetCorrMatrix[key2][key]
                    )
                newDatasetNewDataset_file.write("\n")
        ...
        group_file.write(
            "Groups, Counts, Rates (Hz), Pure Counts, Pure Rates (Hz), Shared Counts, Shared Rates (Hz)\n"
        )
        for key in list(groupCounts.keys()):
            group_file.write(
                ...
                + str(groupCounts[key])
                + ", "
                + str(groupCountsPure[key])
                + ", "
                + str(groupCountsPure[key])
                + ", "
                + str(groupCountsShared[key])
                + ", "
                + str(groupCountsShared[key])
            )
            group_file.write("\n")
