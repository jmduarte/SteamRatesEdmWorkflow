...
parser.add_option(
    ...
    type="str",
    default="noenv",
    help="DIRECTORY where the top of a CMSSW release can be found : /bla/bla/CMSSW_X_X/src",
    metavar="DIRECTORY",
)
parser.add_option(
    "-i",
    ...
    type="str",
    default="no",
    help="In case you want to rerun the make_filesInput.py script, specify the DIR where the input files are located",
    metavar="DIR",
)
parser.add_option(
    "-f",
    ...
    type="str",
    default="custom",
    help="ARG='custom' (default option) or 'RAW', use 'custom' if you're running on STEAM-made files, 'RAW' if you're running on raw data",
    metavar="ARG",
)
parser.add_option(
    "-n",
...
parser.add_option(
    ...
    type="str",
    default="nomaps",
    help="ARG='nomaps' (default option, don't use maps to get dataste/groups/etc. rates), 'somemaps' (get dataset/groups/etc. rates but with no study of dataset merging), 'allmaps' (get dataset/groups/etc. rates and also study dataset merging)",
    metavar="ARG",
)
parser.add_option(
    "-x",
...
if opts.inputFilesDir != "no":
    ...
    print(
        "Making a new filesInputData.py with input root files from %s"
        % opts.inputFilesDir
    )
...
for infile in fileInputNames:
    ...
    tmp_job.write("cd -\n")
    tmp_job.write(
        "python3 triggerCountsFromTriggerResults.py -i %s -j %s -s %s -f %s -m %s -x %s\n"
        % (infile, opts.jsonFile, str(i), opts.fileType, opts.maps, opts.exclude)
    )
